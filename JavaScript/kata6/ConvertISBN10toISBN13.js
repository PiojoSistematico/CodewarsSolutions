...
function isbnConverter(isbn) {
  ...
  let checkDigit= 0  
  let checkString = isbn13.split("-").join("")
  
  for(let i=0;i<checkString.length;i++){
    if(i % 2 === 0){
      checkDigit += parseInt(checkString[i])
    } else {
      checkDigit += parseInt(checkString[i])*3
    }
  }
  let validation = checkDigit % 10
  
  if(validation === 0){
    return isbn13+"0";
  } else {
    return isbn13+(10-validation).toString();
  }
}
